...
export default function WebsiteKpiRow({ ga4, compareLabel }: Props) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      <KpiCard
        ...
        tooltip={`Total sessions ${compareLabel} (GA4)`}
      />
      {ga4.purchaseRevenue > 0 && (
        <KpiCard
          ...
          tooltip={`Purchase revenue ${compareLabel} (GA4)`}
        />
      )}
      {ga4.transactions > 0 && (
        <KpiCard
          ...
          tooltip={`Transactions ${compareLabel} (GA4)`}
        />
      )}
    </div>
  )
